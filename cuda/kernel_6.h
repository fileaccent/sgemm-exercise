...
__global__ void gemm_kernel6(float *d_A, float *d_B, float *d_C, int M, int N, int K) {
    ...
    int d_A_index = (M_tile_index * m + A_m_index) * K + A_n_index;
    int d_B_index = (B_m_index) * N + N_tile_index * n + B_n_index;
    int pre_thread_num;
    int ix;
...
#define prefetch6_1()\
    ...
    FLOAT4(B_sh[B_sh_offset + ix])\
    mid_B = FLOAT4(d_B[d_B_index]);
__global__ void gemm_kernel6_1(float *d_A, float *d_B, float *d_C, int M, int N, int K) {
    const int padding = 4; 
    // 16 * 16 = 256
    // m * k = 1024 一个线程读 4 个
    const int m = 64;
    ...
    do {
        ...
        if (K_tile_index + 1 < int((K + k - 1) / k)) {
            init(&bar, 1);
            cuda::memcpy_async(&A_sh[A_sh_offset^1][A_m_index][A_n_index], &d_A[d_A_index + (K_tile_index + 1) * k], 16, bar);
            cuda::memcpy_async(&B_sh[A_sh_offset^1][B_m_index][B_n_index], &d_B[d_B_index + (K_tile_index + 1) * k * N], 16, bar);
            // FLOAT4(A_sh[A_sh_offset^1][A_m_index][A_n_index]) = FLOAT4(d_A[d_A_index + (K_tile_index + 1) * k]);
            // FLOAT4(B_sh[A_sh_offset^1][B_m_index][B_n_index]) = FLOAT4(d_B[d_B_index + (K_tile_index + 1) * k * N]);
        }
        for (int k_reg_index = 0;;k_reg_index < k; k_reg_index++) {
            for (int i = 0; i < TM; i++) {
                for (int j = 0; j < TK; j++) {
                    reg_A[i][j] = A_sh[A_sh_offset][C_m_index * TM + i][k_reg_index + j];
                }
            }
            for (int i = 0; i < TK; i++) {
                for (int j = 0; j < TN; j++) {
                    reg_B[i][j] = B_sh[B_sh_offset][k_reg_index + i][C_n_index * TN + j];
                }
            }
            for (int i = 0; i < TM; i++) {
                for (int j = 0; j < TN; j++) {
...
float test6_1 () {
    ...
    dim3 thread((thread_size + TM * TN - 1) / (TM * TN));
    int shared_size = sizeof(float) * (m * (k + padding) + k * (n + padding)) * 2;
    gemm_kernel6_1<<<block, thread, shared_size>>>(d_A, d_B, d_C, M, N, K);
    KernelErrChk();
    ErrChk(cudaEventRecord(start, 0));
    for (int i = 0; i < iteration; i++) {
        gemm_kernel6_1<<<block, thread,  shared_size>>>(d_A, d_B, d_C, M, N, K);
...
__global__ void gemm_kernel6_2(float *d_A, float *d_B, float *d_C, int M, int N, int K) {
    const int padding = 4; 
    // 16 * 16 = 256
    // m * k = 1024 一个线程读 4 个
    const int m = 64;
    ...
    int K_tile_index = 0;	
    if (threadIdx.y == 0) {
        FLOAT4(A_sh[A_sh_offset][A_m_index][A_n_index]) = FLOAT4(d_A[d_A_index + K_tile_index * k]);
        FLOAT4(B_sh[A_sh_offset][B_m_index][B_n_index]) = FLOAT4(d_B[d_B_index + K_tile_index * k * N]);
    }
    do {
        __syncthreads();
        if (threadIdx.y == 1) {
...
float test6_2 () {
    ...
    int thread_size = min(m * n, C_size);
    dim3 block((M + m - 1) / m, (N + n - 1) / n);
    dim3 thread((thread_size + TM * TN - 1) / (TM * TN), 2);
    int shared_size = sizeof(float) * (m * (k + padding) + k * (n + padding)) * 2;
    gemm_kernel6_2<<<block, thread, shared_size>>>(d_A, d_B, d_C, M, N, K);
    KernelErrChk();
    ErrChk(cudaEventRecord(start, 0));
    for (int i = 0; i < iteration; i++) {
        gemm_kernel6_2<<<block, thread,  shared_size>>>(d_A, d_B, d_C, M, N, K);
