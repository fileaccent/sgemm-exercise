...
__global__ __launch_bounds__(256) void gemm_kernel7_6(float *d_A, float *d_B, float *d_C, int M, int N, int K) {
    // 16 * 16 = 256
    // m * k = 1024 一个线程读 4 个
    const int m = 64;
    const int n = 64;
    ...
    do {
	    ...
	    FLOAT2(reg_A[reg_offset][0]) = FLOAT2(A_sh[A_sh_offset + A_index]);
            FLOAT2(reg_A[reg_offset][1]) = FLOAT2(A_sh[A_sh_offset + A_index + (k + padding)]);
            FLOAT2(reg_A[reg_offset][2]) = FLOAT2(A_sh[A_sh_offset + A_index + 2 * (k + padding)]);
            FLOAT2(reg_A[reg_offset][3]) = FLOAT2(A_sh[A_sh_offset + A_index + 3 * (k + padding)]);
            FLOAT2(reg_B[reg_offset][0][0]) = FLOAT2(B_sh[B_sh_offset + B_index]);
            FLOAT2(reg_B[reg_offset][0][1]) = FLOAT2(B_sh[B_sh_offset + B_index + 2]);
            FLOAT4(reg_B[reg_offset][1][0]) = FLOAT4(B_sh[B_sh_offset + B_index + (n + padding)]);
            FLOAT4(reg_B[reg_offset][1][1]) = FLOAT4(B_sh[B_sh_offset + B_index + (n + padding) + 2]);
            
	    FLOAT2(reg_A[reg_offset^1][0]) = FLOAT2(A_sh[A_sh_offset + A_index + TK]);
            FLOAT2(reg_B[reg_offset^1][0][0]) = FLOAT2(B_sh[B_sh_offset + B_index + TK * (n + padding)]);
            FLOAT2(reg_B[reg_offset^1][0][1]) = FLOAT2(B_sh[B_sh_offset + B_index + TK * (n + padding) + 2]);

            lgkmcnt<0>();
            do {
                if (k_reg_index + TK < k) {
		    FLOAT2(reg_A[reg_offset^1][1]) = FLOAT2(A_sh[A_sh_offset + A_index + (k + padding) + TK]);
		    FLOAT2(reg_A[reg_offset^1][2]) = FLOAT2(A_sh[A_sh_offset + A_index + (k + padding) + TK]);
		    FLOAT2(reg_A[reg_offset^1][3]) = FLOAT2(A_sh[A_sh_offset + A_index + (k + padding) + TK]);
		    FLOAT2(reg_B[reg_offset^1][1][0]) = FLOAT2(B_sh[B_sh_offset + B_index + (n + padding) + TK * (n + padding)]);
		    FLOAT2(reg_B[reg_offset^1][1][1]) = FLOAT2(B_sh[B_sh_offset + B_index + (n + padding) + TK * (n + padding) + 2]);
                }
                reg_C[0][0] += reg_A[reg_offset][0].x * reg_B[reg_offset][0][0].x;
                reg_C[0][1] += reg_A[reg_offset][0].x * reg_B[reg_offset][0][0].y;
                reg_C[0][2] += reg_A[reg_offset][0].x * reg_B[reg_offset][0][1].x;
                reg_C[0][3] += reg_A[reg_offset][0].x * reg_B[reg_offset][0][1].y;
    
                reg_C[1][0] += reg_A[reg_offset][1].x * reg_B[reg_offset][0][0].x;
                reg_C[1][1] += reg_A[reg_offset][1].x * reg_B[reg_offset][0][0].y;
                reg_C[1][2] += reg_A[reg_offset][1].x * reg_B[reg_offset][0][1].x;
                reg_C[1][3] += reg_A[reg_offset][1].x * reg_B[reg_offset][0][1].y;
    
                reg_C[2][0] += reg_A[reg_offset][2].x * reg_B[reg_offset][0][0].x;
                reg_C[2][1] += reg_A[reg_offset][2].x * reg_B[reg_offset][0][0].y;
                reg_C[2][2] += reg_A[reg_offset][2].x * reg_B[reg_offset][0][1].x;
                reg_C[2][3] += reg_A[reg_offset][2].x * reg_B[reg_offset][0][1].y;
    
                reg_C[3][0] += reg_A[reg_offset][3].x * reg_B[reg_offset][0][0].x;
                reg_C[3][1] += reg_A[reg_offset][3].x * reg_B[reg_offset][0][0].y;
                reg_C[3][2] += reg_A[reg_offset][3].x * reg_B[reg_offset][0][1].x;
                reg_C[3][3] += reg_A[reg_offset][3].x * reg_B[reg_offset][0][1].y;
                
                reg_C[0][0] += reg_A[reg_offset][0].y * reg_B[reg_offset][1][0].x;
                reg_C[0][1] += reg_A[reg_offset][0].y * reg_B[reg_offset][1][0].y;
                reg_C[0][2] += reg_A[reg_offset][0].y * reg_B[reg_offset][1][1].x;
                reg_C[0][3] += reg_A[reg_offset][0].y * reg_B[reg_offset][1][1].y;
                if (k_reg_index + 2 * TK < k) {
		    FLOAT2(reg_A[reg_offset][0]) = FLOAT2(A_sh[A_sh_offset + A_index + 2 * TK]);
		    FLOAT2(reg_B[reg_offset][0][0]) = FLOAT2(B_sh[B_sh_offset + B_index + 2 * TK * (n + padding)]);
		    FLOAT2(reg_B[reg_offset][0][1]) = FLOAT2(B_sh[B_sh_offset + B_index + 2 * TK * (n + padding) + 2]);
                }
                reg_C[1][0] += reg_A[reg_offset][1].y * reg_B[reg_offset][1][0].x;
                reg_C[1][1] += reg_A[reg_offset][1].y * reg_B[reg_offset][1][0].y;
                reg_C[1][2] += reg_A[reg_offset][1].y * reg_B[reg_offset][1][1].x;
                reg_C[1][3] += reg_A[reg_offset][1].y * reg_B[reg_offset][1][1].y;   
    
                reg_C[2][0] += reg_A[reg_offset][2].y * reg_B[reg_offset][1][0].x;
                reg_C[2][1] += reg_A[reg_offset][2].y * reg_B[reg_offset][1][0].y;
                reg_C[2][2] += reg_A[reg_offset][2].y * reg_B[reg_offset][1][1].x;
                reg_C[2][3] += reg_A[reg_offset][2].y * reg_B[reg_offset][1][1].y;
    
                reg_C[3][0] += reg_A[reg_offset][3].y * reg_B[reg_offset][1][0].x;
                reg_C[3][1] += reg_A[reg_offset][3].y * reg_B[reg_offset][1][0].y;
                reg_C[3][2] += reg_A[reg_offset][3].y * reg_B[reg_offset][1][1].x;
                reg_C[3][3] += reg_A[reg_offset][3].y * reg_B[reg_offset][1][1].y;
                reg_offset ^= 1;
                A_index += TK;
                B_index += TK * (n + padding);
                k_reg_index += TK;
...
__global__ void gemm_kernel7_8(float *d_A, float *d_B, float *d_C, int M, int N, int K) {
    ...
    // const int warp_id_n = warp_id % 1;
    // const int lane_id = idx % WAVE_SIZE;
    // const int lane_id_m = lane_id / 32 / 2; // 4 * 16 的块
    // const int lane_id_n = lane_id % 32 * 2 + lane_id % 2;
    // const int C_m_index = warp_id_m * 4 + lane_id_m; // tile内的4 * 4行号
    // const int C_n_index = warp_id_n * 16 + lane_id_n; // tile内的4 * 4列号
    const int C_m_index = idx >> 4; // tile内的4 * 4行号
    const int C_n_index = idx & 15; // tile内的4 * 4列号
